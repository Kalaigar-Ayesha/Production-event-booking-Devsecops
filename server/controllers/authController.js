...
const dbUnavailable = (res) => {
    ...
    res.status(503).json({
        message:
            'Database is not connected. Set MONGO_URI in server/.env (Atlas connection string or local mongodb://127.0.0.1:27017/eventora), fix Atlas Network Access / DNS if using mongodb+srv, then restart the server.',
    });
    return true;
};
