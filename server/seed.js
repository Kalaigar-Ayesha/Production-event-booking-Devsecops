...
// Match server.js: repo-root .env then server/.env (server wins).
...
// Atlas mongodb+srv SRV lookup often fails on some Windows resolvers; align with server.js.
...
const seedDatabase = async () => {
    try {
        ...
        if (String(error.message || error).includes('querySrv')) {
            console.error(
                'Atlas SRV DNS failed. Try: (1) Atlas → Network Access → allow your IP. (2) server/.env DNS_SERVERS=8.8.8.8,1.1.1.1 (3) USE_SYSTEM_DNS=1 (4) standard mongodb:// connection string from Atlas if available.'
            );
        }
        process.exit(1);
    }
